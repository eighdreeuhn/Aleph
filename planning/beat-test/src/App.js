...
function App () {
  ...
  //instruments and global vars//

  const [visualz, setVisualz] = useState({})
  useEffect(() => {
    let bar = Tone.getContext()
    setVisualz({...{bar: bar}})
  },[Tone.Transport])
  ...
  const beatz = function(time) {
    console.log(visualz)
    console.log(time, Tone.Transport.position)
    kick.triggerAttackRelease('A0', '8n', time + measure/4)
    kick.triggerAttackRelease('A0', '8n', time + measure/2)
    kick.triggerAttackRelease('A0', '8n', time + measure*0.75)
  }
  
  const build = function() {
    Tone.getContext().resume()
    kick.chain(psap, test, gain)
    drumz = new Tone.Loop(beatz, '1m')
  ...
  return (
    <div className='App'>
      <Overlay tone={Tone}/>
        ...
        <button onClick={build}>Beatz!</button>
