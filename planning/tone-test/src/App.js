...
function App () {
  ...
  const playBoi = new Tone.Gain()
  const reverb = new Tone.Reverb(
    {
      wet: 0.5
    }
  )
  const mono = new Tone.Scale()
  const fx = new Tone.AutoWah()
  const test = new Tone.MultibandCompressor({
    lowFrequency: 50,
    highFrequency: 220,
    low: {
      threshold: -10
  ...
  const drumz = new Tone.MembraneSynth().chain(test, playBoi)
  ...
  const handlePhraseSubmit = function (e) {
    ...
    playBoi.connect(reverb)
    drumz.connect(playBoi)
    poly.chain(fx, test, playBoi)
    bass.chain(test, playBoi)
    notes = interpolateNotes(phrase)
    setPhrase('')
    waveform = new Tone.Waveform()
    Tone.Destination.connect(waveform)
    buffer = waveform.getValue(0)
    console.log(buffer)
    cycle = new Tone.Loop(time => 
      {
        counter = Tone.Transport.position.split(':')[0] % notes.length
        console.log(counter, time)
        let palette = generatePalette(notes[counter])
        drumz.triggerAttackRelease('A0', '4n', time+0.5, 3)
        drumz.triggerAttackRelease('A0', '4n', time + 1.5, 3)
        palette.forEach((note, i) => {
          poly.triggerAttackRelease(note, '8n', `+${(1 / palette.length) * i}`, 1)
        })
      }, '1m')
  ...
  if (!playing) {
    controlPanel = (
      <div>
        <h3 style={{ color: 'white' }}>
          Aleph is an experiment in synaesthesia. Search for anything you'd
          like, just like you would with Google. Instead of a list of results,
          Aleph processes your search and turns it into a piece of generative
          music accompanied by a visual representation of the sound itself.
        </h3>
    ...
    controlPanel = <button onClick={stopPlay}>STOP</button>
  }
  ...
  return (
    <div className='App'>
      ...
      <div>
        <Screen status={playing} wave={buffer}/>
      </div>
      <Footer />
    </div>
  )
